...
const double TICKS_PER_METER = 425262; // Original was 2800
...
void Calc_Left(const std_msgs::Int16& leftCount) {
  ...
  // if(leftCount.data != 0) {//&& lastCountL != 0) {
    ...
    // if(leftCount.data>20000 && lastCountL <-20000){
    //   leftTicks = (leftCount.data-32767) + (-32767 - lastCountL);
    // }
    // else if(leftCount.data<-20000 && lastCountL >20000){
    //   leftTicks = (32767 - lastCountL) + (leftCount.data+32767);
    // }
    // else{}
    // if (leftTicks > 10000) {
    ...
    distanceLeft = (2*3.14*WHEEL_RADIUS*leftCount.data)/TICKS_PER_METER;
...
void Calc_Right(const std_msgs::Int16& rightCount) {
  ...
  // if(rightCount.data != 0 ){//&& lastCountR != 0) {
    ...
    distanceRight = (2*3.14*WHEEL_RADIUS*rightCount.data)/TICKS_PER_METER;
...
void publish_quat(tf2_ros::TransformBroadcaster tfBroadcaster) {
  ...
  odomToBaseLink.header.stamp = odomNew.header.stamp;
  odomToBaseLink.header.frame_id = "odom";
  odomToBaseLink.child_frame_id = "base_link";
  odomToBaseLink.transform.translation.x = odomNew.pose.pose.position.x;
  odomToBaseLink.transform.translation.y = odomNew.pose.pose.position.y;
  odomToBaseLink.transform.translation.z = odomNew.pose.pose.position.z;
  odomToBaseLink.transform.rotation.x = q.x();
  odomToBaseLink.transform.rotation.y = q.y();
  odomToBaseLink.transform.rotation.z = q.z();
  odomToBaseLink.transform.rotation.w = q.w();

  // Print out some debug information about the transform
  //ROS_INFO("Transform translation: (%f, %f, %f)", odomToBaseLink.transform.translation.x, odomToBaseLink.transform.translation.y, odomToBaseLink.transform.translation.z);
  //ROS_INFO("Transform rotation: (%f, %f, %f, %f)", odomToBaseLink.transform.rotation.x, odomToBaseLink.transform.rotation.y, odomToBaseLink.transform.rotation.z, odomToBaseLink.transform.rotation.w);
